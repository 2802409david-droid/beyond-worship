...
import { updateSchedule, subscribeToSchedules } from "../data/schedules";
import { subscribeToSongs } from "../data/songs";

function EditSchedule({ scheduleId, onNavigate }) {
  ...
  useEffect(() => {
    ...
    const unsubSongs = subscribeToSongs((songs) => {
      if (isMounted) setAvailableSongs(songs || []);
    });

    const unsubSchedules = subscribeToSchedules((schedules) => {
      const found = schedules.find((s) => String(s.id) === String(scheduleId));
      if (found && isMounted) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (onNavigate) onNavigate("schedule-detail", scheduleId);
    } catch (error) {
  ...
  if (loading) {
    return (
      <section className="song-detail-page" style={{ maxWidth: "800px", margin: "0 auto", padding: "40px 20px", textAlign: "center" }}>
        <h3 style={{ color: "#fff" }}>Cargando programación... ☁️</h3>
      </section>
    );
  }
  ...
  return (
    <section className="new-song-page" style={{ maxWidth: "800px", margin: "0 auto", padding: "40px 20px" }}>
      <button
        ...
        onClick={() => onNavigate && onNavigate("schedule-detail", scheduleId)}
      ...
      <div className="new-song-header" style={{ marginBottom: "24px" }}>
        <span className="page-label" style={{ color: "#38bdf8", fontWeight: "700", textTransform: "uppercase", fontSize: "0.85rem", letterSpacing: "0.05em" }}>Edición</span>
        <h2 style={{ margin: "6px 0", fontSize: "2rem", color: "#fff" }}>Editar Programación</h2>
        <p style={{ color: "#94a3b8", margin: "0" }}>Modifica los datos del culto o ensayo.</p>
      </div>
      ...
      <form className="song-form" onSubmit={handleSubmit}>
        ...
        <div className="form-section" style={{ marginBottom: "20px" }}>
          ...
          <p style={{ color: "#94a3b8", fontSize: "0.9rem", marginBottom: "12px" }}>Modifica las canciones para este servicio:</p>
